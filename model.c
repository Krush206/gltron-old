...
Mesh* loadModel(const char *filename, float size, int flags) {
  ...
  while(fgets(buf, sizeof(buf), f)) {
    ...
    if(path == 0) {
      fprintf(stdout, "fatal: can't find mtllib '%s'\n", namebuf);
      exit(1);
    }
    matCount = loadMaterials(path, &materials);
    if(matCount <= 0) {
      fprintf(stdout, "fatal: no Materials loaded\n");
      exit(1);
    } else {
    ...
    fprintf(stdout, "warning: ignored line %d\n", iLine);
      break;
  ...
  for(i = 0; i < matCount; i++) {
    ...
    *(meshFacesize + pos) = 0;
    ...
    pos++;
    if(pos > pMatCount[i]) {
      fprintf(stdout, "fatal: more faces than accounted for\n");
      exit(1);
    }
      }
    }
