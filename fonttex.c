...
fonttex *ftxLoadFont(char *filename) {
  ...
  if(path == 0) {
    fprintf(stdout, FTX_ERR "can't load font file '%s'\n", filename);
    return 0;
  }
  file = fopen(path, "r");
  free(path);
...
void ftxEstablishTexture(fonttex *ftx, unsigned char setupMipmaps) {
  fprintf(stdout,"this should not get called!\n");
}
...
void ftxRenderString(fonttex *ftx, char *string, int len) {
  ...
  for(i = 0; i < len; i++) {
    ...
    if(index >= ftx->upper) 
      fprintf(stdout, FTX_ERR " index out of bounds");
    tex = index / (w * 
w);
    /* bind texture */
    if(tex != bound) {
    ...
    /* draw quad */
    /* fprintf(stdout, FTX_ERR "coords: tex %d (%.2f, %.2f), %.2f\n", */
    /*     bound, cx, cy, cw); */
