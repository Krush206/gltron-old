...
@implementation GLtron (Graphics)
...
- (void) rasterizerOnlyWithDisplay: (gDisplay *) d
{
  ...
  [self checkGLErrorWithSignature: "rasterizerOnlyWithDisplay:"];
...
#define FPS_HSIZE 20
  ...
  [self drawTextWithX: d->vp_w - 180 y: d->vp_h - 20 size: 10 text: tmp];
  sprintf(tmp, "minimum FPS: %d", fps_min);
  [self drawTextWithX: d->vp_w - 180 y: d->vp_h - 35 size: 10 text: tmp];
}
         ...
         text: (char *) text
{
  ...
  [self renderStringWithFontTexture: ftx
        string: text
        length: strlen(text)];
...
- (void) colorDisc
{
  ...
  glVertex3f(0, 0, 0);
  for(h = 0; h <= 360; h += 10) {
    [self HSVToRGBWithH: h s: 1 v: 1 r: &r g: &g b: &b];
    glColor3f(r, g, b);
    /* printf("%.2f %.2f %.2f\n", r, g, b); */
