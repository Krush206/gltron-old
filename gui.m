...
@implementation GLtron (GUI)
- (void) guiProjectionWithX: (int) x y: (int) y
{
  [self checkGLErrorWithSignature: "guiProjectionWithX:y: - start"];
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  /*glOrtho(0, 0, x, y, -1, 1); */
  [self checkGLErrorWithSignature: "guiProjectionWithX:y: - proj"];
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  glViewport(0, 0, x, y);
  [self checkGLErrorWithSignature: "guiProjectionWithX:y: - end"];
}
...
#define N 20.0
  [self checkGLErrorWithSignature: "displayGui - before clear"];
  glClearColor(0.0, 0.0, 1.0, 0.0);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  [self guiProjectionWithX: game->screen->vp_w y: game->screen->vp_h];
  ...
  [self checkGLErrorWithSignature: "displayGui"];
  ...
  [self drawMenuWithDisplay: game->screen];
...
- (void) keyboardGuiWithKey: (unsigned char) key
         x: (int) x
         y: (int) y
{
  int i;
  ...
  case 27:
    ...
    break;
  case 13: case ' ':
    [self menuActionWithMenu: *(pCurrent->pEntries + pCurrent->iHighlight)];
    break;
  case 'q': exit(0); break;
  case 'l':
    printf("%d entries:\n", pCurrent->nEntries);
