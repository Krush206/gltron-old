...
png_texture* load_png_texture(char *filename) {
  ...
  if(bpc != 8) {
    png_destroy_read_struct(&png_ptr, &info_ptr, 0);
    fprintf(stdout, ERR_PREFIX "wrong bitdepth: %d\n", bpc);
    fclose(f);
    return 0;
  }
