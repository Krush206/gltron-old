...
void menuAction(MMenu *activated, int type) {
  int x, y;
  char c;
  int *piValue;
  if(activated->nEntries > 0) {
    ...
    case 'q': SystemExit(); saveSettings(); exit(0); break;
    case 'r': 
      initData();
...
MMenu* loadMenu(FILE* f, char* buf, MMenu* parent, int level) {
  MMenu* m;
  int i;
  ...
  if(m->nEntries > 0) { /* contains a submenu */
    m->pEntries = malloc(sizeof(MMenu*) * m->nEntries);
...
MMenu** loadMenuFile(char *filename) {
  ...
  list = (MMenu**) malloc(sizeof(MMenu*) * nMenus);
...
#define MENU_TEXT_LINEHEIGHT 1.5
  ...
  for(i = 0; i < pCurrent->nEntries; i++) {
    int len;
    len =  strlen(((MMenu*)*(pCurrent->pEntries + i))->display.szCaption);
    if(len > maxw)
      maxw = len;
  }
  ...
  for(i = 0; i < pCurrent->nEntries; i++) {
    ...
    drawText(guiFtx, x, y, size,
         ((MMenu*)*(pCurrent->pEntries + i))->display.szCaption);
    y -= lineheight;
  }
