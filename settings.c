...
void initMainGameSettings(char *filename) {
  ...
  if(home == NULL) {
    ...
    sprintf(fname, "%s%s", home, RC_NAME);
  }
  f = fopen(fname, "r");
  if(f == 0)
  ...
  if(game->settings->speed <= 0) {
    fprintf(stdout, "[gltron] sanity check failed: speed = %.2ff\n",
        game->settings->speed);
    game->settings->speed = 6.0;
    fprintf(stdout, "[gltron] reset speed: speed = %.2f\n",
        game->settings->speed);
  }
  if(game->settings->grid_size % 8) {
    fprintf(stdout, "[gltron] sanity check failed: grid_size %% 8 != 0: "
        "grid_size = %d\n", game->settings->grid_size);
    game->settings->grid_size = 240;
    fprintf(stdout, "[gltron] reset grid_size: grid_size = %d\n",
        game->settings->grid_size);
  }
