...
sgi_texture* load_sgi_texture(char *filename) {
  ...
  f = fopen(filename, "r");
  if(f == 0) {
    fprintf(stdout, ERR_PREFIX "can't open file %s\n", filename);
    return 0;
  }
  
  fread(buf, 512, 1, f);
  if((buf[0] << 8) + (buf[1] << 0) != 474) {
    fprintf(stdout, ERR_PREFIX "wrong magic: %d %d\n",
        buf[0], buf[1]);
    return 0;
  }
  
  if(buf[2] != 0) {
    fprintf(stdout, ERR_PREFIX "RLE compression not supported\n");
    return 0;
  }

  if(buf[3] != 1) {
    fprintf(stdout, ERR_PREFIX "BPC is %d - not supported\n", buf[3]);
    return 0;
  }
  ...
  if(zsize != 3 && zsize != 4) {
    fprintf(stdout, ERR_PREFIX "number of channels is %d - not supported\n",
        zsize);
    return 0;
  }
  ...
  count = x * y * zsize * bpc;
  /* fprintf(stdout, ERR_PREFIX "loading %ld bytes\n", count); */
