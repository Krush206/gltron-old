         ...
         y: (int) y
{
  switch (k) {
  case 'q': exit(0); break;
  case 27: [self switchCallbacksWithCallbacks: &guiCallbacks]; break;
    /* steering player 0 */
  case 'a': case 'A': [self turnWithData: game->player[0].data
                            direction: 3]; break;
  case 's': case 'S': [self turnWithData: game->player[0].data
                            direction: 1]; break;
    /* steering player 0 */
  case 'k': case 'K': [self turnWithData: game->player[1].data
                            direction: 3]; break;
  case 'l': case 'L': [self turnWithData: game->player[1].data
                            direction: 1]; break;
    /* steering player 2 */
  case '5': [self turnWithData: game->player[2].data
                  direction: 3]; break;
  case '6': [self turnWithData: game->player[2].data
                  direction: 1]; break;
    /* steering player 3 */
    /* cursor keys in specialKey() */
  case ' ': [self switchCallbacksWithCallbacks: &pauseCallbacks]; break;
         ...
         y: (int) y
{
  ...
  case GLUT_KEY_LEFT: [self turnWithData: game->player[3].data
                            direction: 3]; break;
  case GLUT_KEY_RIGHT: [self turnWithData: game->player[3].data
                             direction: 1]; break;
