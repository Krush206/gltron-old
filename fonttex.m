...
@implementation GLtron (FontTexture)
...
- (fonttex *) loadFontWithFile: (char *) filename
{
  ...
  [self getLineWithBuffer: buf size: sizeof buf file: file];
  ...
  for(i = 0; i < ftx->nTextures; i++) {
    ...
    path = [self fullPathWithFile: texname];
    if(path == 0) {
      // clean up allocated memory & spit out error
      int j;
      for(j = 0; j < i; j++)
	[self unloadSGITextureWithSGITexture: *(ftx->textures + j)];
      free(ftx->textures);
      free(ftx->fontname);
      free(ftx);
    ...
    *(ftx->textures + i) = [self loadSGITextureWithFile: path];
    free(path);
  }
  return ftx;
}
...
- (void) unloadFontWithFontTexture: (fonttex *) ftx
{
  int i;
  for(i = 0; i < ftx->nTextures; i++)
    [self unloadSGITextureWithSGITexture: *(ftx->textures + i)];
  free(ftx->textures);
  free(ftx->texID);
  free(ftx->fontname);
  free(ftx);
