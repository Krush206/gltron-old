...
void restoreCallbacks() {
  if(last_callback == 0) {
    fprintf(stdout, "no last callback present, exiting\n");
    exit(1);
  }
  ...
  fprintf(stdout, "restoring callbacks\n");
}
...
/* TODO(3): incorporate model stuff */
  ...
  if(strcmp(name, "chooseModel") ==  0) {
    fprintf(stdout, "change callbacks to chooseModel\n");
    switchCallbacks(&chooseModelCallbacks);
  }
  */
  if(strcmp(name, "gui") == 0) {
    /* fprintf(stdout, "change callbacks to gui\n"); */
