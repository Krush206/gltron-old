...
- (void) turnWithData: (Data *) data direction: (int) direction
{
  ...
  if(data->speed > 0) { /* only allow turning when in-game */
    ...
    data->turn_time = [self getElapsedTime];
...
- (void) changeDisplay
{
  ...
  for(i = 0; i < vp_max[game->settings->display_type]; i++)
  {
    ...
    content = game->settings->content[i];
    [self initDisplayWithDisplay: game->player[content].display
          type: game->settings->display_type
          player: i
          onScreen: 1];
  }
}
...
{ /* called only once */
  ...
  for(i = 0; i < game->players; i++) {
    ...
    path = [self fullPathWithFile: "t-u-low.obj"];
    // path = getFullPath("tron-med.obj");
    if(path != 0)
      // model size == CYCLE_HEIGHT
      p->model->mesh = [self loadModelWithFile: path
                             size: CYCLE_HEIGHT
                             flags: 1];
    else {
    ...
    [self setAmbientWithMesh: p->model->mesh
          material: 0
          color: p->model->color_model];
    [self setDiffuseWithMesh: p->model->mesh
          material: 0
          color: p->model->color_model];
...
- (void) initData
{
  ...
  for(i = 0; i < game->players; i++) {
    ...
    [self setAlphaWithMesh: model->mesh alpha: 1.0];
...
- (void) fixTrails
{
  ...
  sel = @selector(setColWithX:y:);
  mark = [self methodFor: sel];
  for(i = 0; i < game->players; i++) {
    d = game->player[i].data;
    if(d->speed > 0) {
      t = &(d->trails[0]);
      while(t != d->trail) {
        [self doTrailWithLine: t
              mark: mark
              selector: sel];
	t++;
      }
...
- (void) idleGame
{
  ...
  if([self getElapsedTime] - lasttime < 10 && loop == 1) return;
  [self timeDiff];
  for(j = 0; j < loop; j++) {
    if(loop == FAST_FINISH)
      dt = 20;
	...
	if(game->player[i].ai->active == 1)
          [self doComputerWithPlayer: &(game->player[i])
                data: game->player[i].data];
  }
...
- (void) movePlayers
{
  ...
  for(i = 0; i < game->players; i++) {
	...
	/* collision-test here */
	/* boundary-test here */
	col = [self collDetectWithX: data->posx
                    y: data->posy
                    x: newx
                    y: newy
                    direction: data->dir
